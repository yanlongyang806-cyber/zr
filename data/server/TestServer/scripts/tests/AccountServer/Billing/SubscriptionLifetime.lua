...
while i <= Loop do
	print("---Loop: ", i);
	-----Create Account1
	Account1 = AccountServer.Test.CreateAccount()

	--Apply Retail Key
	--Add Lifetime with CC
	AccountServer.Test.SubscribeAccount(Account1, SubLifetime, InteralProduct, RetailKeys[I])


	-----Create Account2
	Account2 = AccountServer.Test.CreateAccount()


	--Apply Retail Key
	--Add 1 Mo Sub with CC
	AccountServer.Test.SubscribeAccount(Account2, Sub1Mo, InteralProduct, RetailKeys[i+2])

	--Add Lifetime sub
	print("\nApplying Lifetime sub, verifying Vindicia sub is cancelled...");
	AccountServer.Test.SubscribeAccount(Account2, SubLifetime, InteralProduct)

	-----Create Account3
	Account3 = AccountServer.Test.CreateAccount()


	--Apply Retail Key, Add GTC
	AccountServer.Test.SubscribeAccount(Account3, GTCSub, InteralProduct, RetailKeys[i+3], GTCKeys[i])

	--Add Lifetime sub
	AccountServer.Test.SubscribeAccount(Account3, SubLifetime, InteralProduct)
	i = i+1
end
