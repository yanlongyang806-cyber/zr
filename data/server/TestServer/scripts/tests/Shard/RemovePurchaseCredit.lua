...
Test.Require(shard, "Please specify the target shard in the variable \"RemovePurchaseCredit:Shard\".");

local user = Scope.Var.Get("User");
Test.Require(user, "Please specify your Cryptic account name in the variable \"RemovePurchaseCredit:User\".");

local pass = Scope.Var.Get("Password");
Test.Require(pass, "Please specify your Cryptic account password in the variable \"RemovePurchaseCredit:Password\".");

local path = Scope.Var.Get("ImportFile");
Test.Require(path, "Please specify the full path to the fixup CSV in \"RemovePurchaseCredit:ImportFile\".");
...
function FixupChar(i, char)
	...
	local r, t = WebRequestXMLRPC("MicroTrans_RemoveOneTimePurchase", char.character, char.purchase);
	if not r then
...
local chars = Console.ReadCSVFile(path, "character", "purchase");
