...
local tinsert, tremove, pairs = table.insert, table.remove, pairs
local assert, type, print, tostring = assert, type, print, tostring
local lxp = lxp
...
function  parse (o)
  ...
  elseif type(o) == "table" then
    for _,l in pairs(o) do
      status, err = p:parse(l)
      if not status then return nil, err end
    end
  else
    status, err = p:parse(tostring(o))
    if not status then return nil, err end
  end
  status, err = p:parse()
  if not status then return nil, err end
